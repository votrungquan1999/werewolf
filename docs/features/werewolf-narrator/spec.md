...
- **Day votes are passed around, not open.** The phone goes to each living player in turn behind the same hand-off gate as the night, and anyone may abstain. The running tally stays hidden until everyone has had the phone — showing it live would tell each voter exactly how everyone before them voted. A tie forces a revote between the tied players; a second tie kills nobody.
- **The day opens with a timed argument.** Daybreak crows a rooster, the dead are revealed only on a tap, and then a 2-minute countdown runs before voting, extendable by the minute. It never auto-advances — the table is usually mid-argument, so a human decides when to move on.
